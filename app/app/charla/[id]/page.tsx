...
// One main-app voice conversation, in full — the Speak-side sibling of
// /grow/voice/[id]: session review above, transcript below, tap a partner
// turn for the English.
...
import type { VoiceSessionRecord, VoiceTurnRecord } from "@/lib/voice/types"
import { ensureSpeakSessionAnalysis, getVoiceSession, listVoiceTurns } from "@/lib/voice/store"
...
function formatMeta(session: VoiceSessionRecord): string {
  const parts: string[] = []
...
export default function CharlaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  return (
    <div className="min-h-dvh bg-background px-4 py-6 pb-24">
      ...
      {turns.some((t) => t.speaker === "user") && (
        <SessionReview sessionId={session.id} analyze={ensureSpeakSessionAnalysis} />
      )}
