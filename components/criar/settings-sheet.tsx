...
export function SettingsSheet({ open, onOpenChange, child, onChildUpdate }: SettingsSheetProps) {
  ...
  const preview = async (id: VoiceId) => {
    ...
    setPreviewingId(id)
    try {
      const audio = await playAudio(ttsUrl(SAMPLE_TEXT, id, "rioplatense"))
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto rounded-t-2xl px-6 pb-8">
        ...
        <div className="mb-6 space-y-3">
          ...
          <div>
            <label htmlFor="grow-child-name" className="mb-1 block text-xs text-muted-foreground">
              Name
            </label>
            <input
              id="grow-child-name"
          ...
          <div>
            <label htmlFor="grow-child-birthdate" className="mb-1 block text-xs text-muted-foreground">
              Birth date
            </label>
            <input
        ...
        <div className="space-y-2">
          <h3 className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Voice</h3>
          {voices.map((voice) => {
